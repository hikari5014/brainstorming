...
| 中文語音生成要等（約等於說話長度）＝實際上等不起 | **對方→中文預設純文字**：TEXT modality（不支援時降級 AUDIO 並丟棄），讀字幕零等待、額度省一半；我說的話仍翻成外語語音給對方 |
| 閒置連線佔額度、久連品質劣化 | **每輪結束確實斷線**（預設）：播完/字幕完即斷，按住說話時自動重連，開頭進緩衝不漏字；另支援**雙 API key**（每方向一把，額度加倍） |
| 對方沒聽清楚只能整句重講（再耗一次額度） | **重播鍵**：外語語音留存本機（120 秒上限），對方側字幕旁一鍵重播，零網路、零額度 |
