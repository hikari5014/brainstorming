...
| 常用句每次都要重新說、重新翻譯 | **常用句庫**（中控列 ☆ 收藏／📖 開啟、首頁可管理）：中文＋外語＋語音一起存 IndexedDB，點擊即播＋大字顯示，離線可用、零額度；依語言自動分類、可置頂；**外語語音跟著逐字稿保存**，事後從逐字稿逐句轉存也帶語音 |
| 即時翻譯逐段生成 → 字幕生硬、糊成一團 | **聆聽模式兩段式字幕**：粗譯即時上、句子完成後 AI 依上下文潤飾原地替換（綠色虛線＋✨，可關）；句句換行；講者語言改變自動標注 🌐 |
| 長原文擠壓中文翻譯 | **上下雙視窗**：上＝中文、下＝原文，中間分界可拖動調整比例（記憶設定）；原文視窗與段落分隔線皆可關 |
| 設定項目太多難找 | 設定分頁籤：**API 設定**（含診斷）／**對話翻譯**／**聆聽** |
